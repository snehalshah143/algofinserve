package tech.algofinserve.infra;

public class AngelApiKey {

    public final static String marketApiKey="OtbLUg4X";
    public final static String marketSecretKey="11841221-7bf1-4e99-8005-a25ca002a454";

    public final static String historicalApiKey="gX3qXZXn";
    public final static String historicalSecretKey="44064590-d952-44b4-b64b-175fcd1095cb";

    public final static String oneTimetotpKey="RUVZEH5OVV6MDYEWWAQ3EUSGCM";

    public final static String clientID="S133712";
    public final static  String passowrd="1805";

}
